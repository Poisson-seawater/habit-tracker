...
| `/quetes` *(alias `/habitudes`, `/habits`, `/quests`)* | *(aucun)* | Affiche le **panneau paginé des quêtes du jour** : un bouton par habitude prévue (plus celles déjà traitées aujourd'hui), avec son état (⬜ à faire, ✅ validée, 📊 progression, ⏭️ skippée, ❌ ratée). Un clic valide une habitude binaire et actualise le panneau sur place ; une habitude quantitative demande sa valeur avant d'actualiser le panneau. Une quête complétée, skippée ou ratée reste visible mais son bouton devient neutre. Chaque panneau appartient à l'utilisateur qui l'a ouvert. Bouton `🔄 Rafraîchir` en bas. Les quêtes privées s'affichent « Chose secrète 🔒 ». | `/quetes` |
