...
| `/quetes` *(alias `/habitudes`, `/habits`, `/quests`)* | *(aucun)* | Affiche le **panneau des quêtes du jour** : un bouton par habitude prévue (plus celles déjà traitées aujourd'hui), avec son état (⬜ à faire, ✅ validée, 📊 progression, ⏭️ skippée, ❌ ratée). Un clic valide une habitude binaire et rafraîchit le panneau sur place ; une habitude quantitative demande la valeur puis renvoie le panneau à jour. Bouton `🔄 Rafraîchir` en bas. Les quêtes privées s'affichent « Chose secrète 🔒 ». | `/quetes` |
