...
Tu es déjà dans la boucle : tu logges tes habitudes, tu vois ton streak grandir. Cette page couvre l'usage avancé — ce qui ne saute pas aux yeux au premier jour — et répond aux objections qui reviennent le plus souvent.

## Aller plus loin dans le quotidien
...
**« Je vis déjà dans Google Agenda, je ne vais pas ouvrir un site en plus. »** Pas besoin de choisir : connecte ton compte Google et tes primes planifiées apparaissent comme événement ou tâche dans tes outils habituels. Le mapping exact des dates et les rappels sont détaillés dans la [synchronisation Google Calendar & Tasks](#/sync-google).

**« Le Pi est exposé sur Internet, ça ne craint rien ? »** Trois couches filtrent l'accès avant même d'atteindre le dashboard : Cloudflare Access (email autorisé), un mot de passe applicatif par joueur, puis un appareil reconnu. Le détail — y compris l'auto-approbation des appareils depuis le 2 juillet 2026 — est dans [Authentification & appareils](#/authentification).

## Piloter autrement

Le dashboard et le bot Telegram ne sont pas les seules portes d'entrée : un agent IA peut aussi consulter et modifier ton compte directement, sans ouvrir ni le site ni Telegram. Le détail est dans [Télécommande IA](#/telecommande-ia).
