...
def parse_command(text: str) -> dict:
    ...
    if cmd == "/done":
        ...
        if len(log_parts) < 2:
            raise ParserError(
                "Usage : /log [nom_habitude] [valeur][unité]\nExemple : /log lecture 30min"
            )
        ...
        if not args_str:
            raise ParserError(
                "Usage : /skip [nom_habitude] raison: [votre raison]\nExemple : /skip nage raison: fatigue extreme"
            )
        ...
        if not args_str:
            raise ParserError(
                "Usage : /add_habit [binary|quant] [titre] [unité optionnelle]"
            )
        ...
        if filter_type not in [
            ...
            raise ParserError(
                "Usage : /shop [toutes|dispos|verrouillees]\nExemple : /shop dispos"
            )
