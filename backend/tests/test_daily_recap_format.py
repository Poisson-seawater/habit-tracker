...
def setup_test_db():
    ...
    try:
        ...
        # Create some habits
        h1 = Habit(id=1, user_id=1, name="Mediter", type="binary", point_rewards={"discipline": 1}, frequency="daily")
        h2 = Habit(id=2, user_id=1, name="Lecture", type="quantitative", point_rewards={"apprendre": 2}, unit="pages", frequency="daily")
        db.add(h1)
        db.add(h2)

        # Create a Todo
        t = Todo(id=1, user_id=1, title="Faire la vaisselle", is_completed=True, completed_at=datetime.datetime.now())
        db.add(t)

        # Create a NoTodo (failed)
        nt = NoTodo(id=1, user_id=1, title="Manger du fastfood", failed_at=datetime.datetime.now())
        db.add(nt)

        # Add habit logs for today
        log1 = HabitLog(user_id=1, habit_id=1, log_type="done", timestamp=datetime.datetime.now())
        log2 = HabitLog(user_id=1, habit_id=2, log_type="log", amount=15, unit="pages", timestamp=datetime.datetime.now())
        db.add(log1)
        db.add(log2)
...
def test_daily_recap_formatting(monkeypatch):
    ...
    assert "Hab habitudes faites" or "Habitudes faites :</b> Mediter ✅, Lecture (15pages)" in recap_msg
