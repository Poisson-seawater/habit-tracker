...
def test_telegram_webapp_session_resolves_existing_user():
    response = client.post(
        ...
        json={"id": 111, "username": "Gabriel", "first_name": "Gabriel"},
    )
    assert response.status_code == 200
    data = response.json()
    assert data["id"] == 1
...
def test_telegram_webapp_session_does_not_rename_existing_chat_id():
    response = client.post(
        "/api/v1/telegram-webapp/session",
        json={"id": 111, "username": "PandaCoffey", "first_name": "Gabriel"},
    )
    assert response.status_code == 200
    data = response.json()
    assert data["id"] == 1
...
def test_telegram_webapp_session_creates_unknown_user():
    response = client.post(
        "/api/v1/telegram-webapp/session", json={"id": 333, "first_name": "Alice"}
    )
    assert response.status_code == 200
    data = response.json()
...
def test_post_logs_endpoint():
    response = client.post("/api/v1/logs", json={"habit_id": 1, "log_type": "done"})
    assert response.status_code == 200
    data = response.json()
...
def test_goals_and_substeps_crud():
    ...
    response = client.post(
        "/api/v1/goals",
        json={"title": "Devenir Millionnaire", "description": "Atteindre 1M en actif"},
    )
    assert response.status_code == 201
    ...
    response = client.put(
        ...
        json={"title": "Devenir Milliardaire", "description": "Atteindre 1B en actif"},
    )
    assert response.status_code == 200
    data = response.json()
    ...
    try:
        ...
        db.add(
            HabitLog(
                ...
                habit_id=2,
                log_type="skip",
                reason="Pas le temps aujourd'hui",
                timestamp=noon_today,
            )
        )
        db.commit()
    finally:
...
def test_per_goal_substep_execution_order():
    ...
    resp_s = client.post(
        f"/api/v1/goals/{goal_a_id}/substeps",
        json={
    ...
    resp_link = client.post(
        ...
        json={"goal_id": goal_b_id, "substep_id": substep_id, "execution_order": 5},
    )
    ...
    resp_reorder = client.put(
        f"/api/v1/goals/{goal_a_id}/substeps/{substep_id}/reorder",
        json={"execution_order": 4},
    )
...
def test_notodo_crud():
    # 1. Create a No-Todo
    response = client.post("/api/v1/notodos", json={"title": "Ne pas procrastiner"})
    assert response.status_code == 201
    data = response.json()
    assert data["status"] == "success"
