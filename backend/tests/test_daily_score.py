...
def test_daily_score_calculation_incomplete(db_session):
    ...
    score = calculate_daily_score(db_session, user_id=1, date=today, template_name="week")
...
def test_streak_increment_on_perfect_day(db_session):
    ...
    streak = Streak(user_id=1, streak_type="Perfect", current_streak=2, max_streak=2, last_incremented=yesterday)
    db_session.add(streak)
    db_session.commit()
    
    # When perfect score is registered today
    score = DailyScore(user_id=1, date=today, status="Perfect", template_used="week", actual_stats={})
    ...
    updated_streak = db_session.query(Streak).filter_by(user_id=1, streak_type="Perfect").first()
